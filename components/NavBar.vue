...
<template>
  <header
    ...
    {{user}}
    <NuxtLink class="text-3xl font0mono text-yellow-400" to="/">cartrader</NuxtLink>
    <div v-if="user" class="flex space-x-2">
      <NuxtLink to="/profile/listings" class="text-white mr-5">Listings</NuxtLink>
      <p @click="logout" class="text-white mr-5 cursor-pointer">Logout</p>
    </div>
    <div v-else class="flex space-x-2">
      <NuxtLink to="/login" class="text-white mr-5">Login</NuxtLink>
    </div>
  </header>
</template>